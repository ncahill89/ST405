...
## Components of a generalized linear model {style="font-size: 65%;"}
...
$$\mbox{log}\left(\frac{\pi}{1-\pi}\right) = \alpha + \beta_{1}x_{1}$$

**Interpretation of** $\alpha$:

At $x_1 = 0$, then \begin{eqnarray*}
\log\left(\frac{\pi}{1-\pi}\right) &=& \alpha \\
\frac{\pi}{1-\pi} &=& e^{\alpha}\\
\end{eqnarray*} So $e^{\beta_0}$ = the **success odds** at $x_1 = 0$.

## Interpretation of logistic regression parameters {style="font-size: 65%;"}

**Interpretation of** $\beta_{1}$:

Suppose $X_1$ increases from $x_1$ to $x_1+1$. Let $w_1$ = the success odds at $x_1$ and $w_2$ = the success odds at $x_1+1$. Then the odds change from: $$w_1 = e^{\alpha + \beta_{1}x_{1}}$$ to \begin{eqnarray*} 
w_2 &=& e^{\alpha + \beta_{1}(x_{1}+1)} \\
&=& w_{1} e^{\beta_{1}}
\end{eqnarray*}

i.e., increasing $X_1$ by 1 unit changes the success odds by a multiplicative factor of $e^{\beta_{1}}$

Or, $$\frac{w_2}{w_1} = e^{\beta_{1}}$$

\vspace{0.3cm}

i.e., $e^{\beta_{1}}$ is the **odds ratio** for a unit increase in $x_1$.
...
## Poisson regression models: count responses {style="font-size: 65%;"}
...
\log(\lambda) &=& \alpha \\
\lambda &=& e^{\alpha}\\
\end{eqnarray*} So $e^{\alpha}$ = the **expected count** at $x_1 = 0$.

## Interpretation of logistic regression parameters {style="font-size: 65%;"}

**Interpretation of** $\beta$:

Suppose $x_1$ increases from $x$ to $x+1$. Then $\lambda$ changes from
...
Each unit increase in $x_1$ multiplies the mean response (expected count) by $e^{\beta_1}$.
