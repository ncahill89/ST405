...
$log(r) = \text{min}(0, \sum_i (dnorm(y_i,\mu_{pro},\sigma, log = T)) + dnorm(\mu_{pro},\mu_0,\sigma_0, log = T)$ $- \sum_i (dnorm(y_i,\mu_{cur},\sigma, log = T)) - dnorm(\mu_{cur},\mu_0,\sigma_0, log = T))$
...
mupost.mean = (tau_s[s]*n*mean(y) + mu0/(sigma.mu0^2))/(tau_s[s]*n + 1/(sigma.mu0^2))
mupost.sd = sqrt(tau_s[s]*n + 1/(1/(sigma.mu0^2)))
