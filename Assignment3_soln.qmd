...
Marks for the correct JAGS model spec. The part for Tasks 3 and 4 doesn't need to be included to answer this question. **\[8 Marks\]**

Summaries are only required for $\alpha$, $\beta$, $\alpha_{\sigma}$ and $\beta_{\sigma}$. **\[8 Marks\]**
...
The correct code for `yrep` added to the JAGS model **\[4 Marks\]**
...
The model is doing a good job at capuring the variation in the outcome accross most values of the predictor. Perhaps the variation is being slightly underestimated at lower values of x. **\[3 Marks\]**

-   [ ] **Task 4: Additional Analysis**

**Notes for marking:**

Give marks for creating the predictive distribution in the JAGS model and look out for the creation of the prediction interval in this model as sigma needs to be created first based on the predictor value and then used in the predictive distribution for y. **\[8 Marks\]**
